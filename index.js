// ===================
// Adalea Tickets v2 Clone - FINAL PRODUCTION FILE (V22 - ANTI-CRASH + USERNAME FIX)
// ===================
import {
    Client,
    GatewayIntentBits,
...
// ===================
// ANTI-CRASH SYSTEM (SAFETY FEATURES)
// ===================
...
const saveTickets = () => {
    try {
        ...
        console.error(' [ERROR] Failed to save ticketData.json:', error);
    }
};
    ...
    try {
        if (fs.existsSync(ticketDataPath)) {
            ...
            if (fileContent.trim().length > 0) {
                ...
                console.log(` [SYSTEM] Loaded active tickets.`);
            } else {
                tickets = {};
            }
        }
...
client.on('messageCreate', async message => {
    ...
    if (content.startsWith('?')) {
        const [cmd, ...args] = content.slice(1).toLowerCase().split(/\s+/);
        const arg = args.join(' ');
            ...
            try {
                ...
                const row = new ActionRowBuilder().addComponents(
                    Object.values(categories).map(c =>
                        new ButtonBuilder()
                            .setLabel(c.name)
                            // FIX: Using Key directly to prevent errors
                            .setCustomId(`category_${c.key}`)
    ...
    // --- !ticketchannel COMMAND ---
    if (content.startsWith('!ticketchannel')) {
        await message.delete().catch(() => {});

        if (!message.member.roles.cache.has(IDs.TICKET_REGISTRAR_ROLE_ID)) {
            const reply = await message.channel.send(`You do not have permission. Required Role: <@&${IDs.TICKET_REGISTRAR_ROLE_ID}>`);
        ...
        let rawUserId = args[2];

        if (!categoryKey || !rawUserId) {
        ...
        // Sanitize ID
        const userId = rawUserId.replace(/\D/g, ''); 

        const category = getCategoryByKey(categoryKey);
        if (!category) {
            const reply = await message.channel.send(`Invalid category key: \`${categoryKey}\`. Valid: ${Object.keys(categories).join(', ')}.`);
            setTimeout(() => reply.delete().catch(() => {}), 5000);
            return;
        }
        ...
        const reply = await message.channel.send(`✅ Channel **#${message.channel.name}** registered as **${category.name}** for <@${userId}>.`);
        setTimeout(() => reply.delete().catch(() => {}), 5000);
    }
});
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton() && interaction.customId.startsWith('category_')) {
        ...
        try {
            // FIX: Using Key directly
            const catKey = interaction.customId.replace('category_', '');

            if (!categories[catKey]) return interaction.editReply({ content: 'Category configuration not found.' });
            ...
            if (category.subtopics) {
                ...
                return interaction.editReply({ content: 'Please select a subtopic.', components: [menu] });
            }

            if (hasCooldown(interaction.user.id))
                return interaction.editReply({ content: `You are on cooldown. Wait ${COOLDOWN_SECONDS}s.` });
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('subtopic_')) {
        ...
        try {
            ...
            if (isSubtopicStopped(catKey, selectedKey)) return interaction.editReply({ content: 'This subtopic is stopped.', components: [] });

            if (hasCooldown(interaction.user.id))
                return interaction.editReply({ content: `You are on cooldown. Wait ${COOLDOWN_SECONDS}s.`, components: [] });
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === 'claim') {
            if (!tickets[channelId]) return interaction.reply({ content: 'Not an active ticket channel.', ephemeral: true });
            ...
            return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x32CD32).setDescription(`✅ <@${interaction.user.id}> **claimed this ticket**.`)] });
        }
        ...
        if (interaction.commandName === 'ticket') {
            ...
            if (!tickets[channel.id]) return interaction.reply({ content: 'Not an active ticket.', ephemeral: true });
            ...
            try {
                if (subcommand === 'add') {
                    ...
                    return interaction.reply({ content: `${user.tag} added.`, ephemeral: false });
                }
                if (subcommand === 'remove') {
                    await channel.permissionOverwrites.edit(user.id, { ViewChannel: false });
                    return interaction.reply({ content: `${user.tag} removed.`, ephemeral: false });
                }
                if (subcommand === 'move') {
                    const newCategoryKey = interaction.options.getString('category_key');
                    const newCat = getCategoryByKey(newCategoryKey);
                    
                    if (!newCat) return interaction.reply({ content: 'Invalid category.', ephemeral: true });
                    ...
                    const moveEmbed = new EmbedBuilder().setColor(0x0099FF).setDescription(`🎟️ Moved from **${oldCat?.name || 'N/A'}** to **${newCat.name}**.`);
                    ...
                    return interaction.reply({ content: `Moved to **${newCat.name}**.`, ephemeral: true });
                }
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('close_modal_')) {
        ...
        try {
            ...
            if (logChannel) {
                await logChannel.send({ embeds: [logEmbed], files: [{ attachment: transcriptPath, name: transcriptFilename }] });
            }
            ...
            try {
                ...
                const dmEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`Your ticket **#${ticketChannel.name}** was closed by **${interaction.user.tag}**.`)
                    .addFields({ name: 'Reason', value: reason, inline: false });
                
                await creator.send({ embeds: [dmEmbed], files: [{ attachment: transcriptPath, name: transcriptFilename }] });
            } catch (dmError) {
                console.warn(`Failed to DM user: ${dmError.message}`);
            }

            fs.unlinkSync(transcriptPath);

            await interaction.editReply({ content: 'Ticket closed. Deleting in 5s...' });
            ...
            ticket.closed = false; 
            saveTickets();
            await interaction.editReply({ content: 'Critical error during close. Ticket remains open.' });
        }
    }
});

// ===================
// TICKET CREATION (USERNAME EDITION)
// ===================
async function createTicketChannel(user, categoryKey, subtopicKey, interaction) {
    const guild = interaction.guild;
    const cat = categories[categoryKey];
    
    // FIX: Using Username instead of Counter for Render stability
    // 1. Get lowercase username, remove special chars
    const cleanUsername = user.username.toLowerCase().replace(/[^a-z0-9]/g, '').substring(0, 20);
    // 2. If username was all special chars (empty now), use ID
    const safeName = cleanUsername.length > 0 ? cleanUsername : user.id.substring(0, 5);
    
    const name = `${categoryKey}-${safeName}`; 
    ...
    try {
        ...
        interaction.editReply({ content: 'Failed to create ticket channel.', components: [] }).catch(() => {});
    }
}
...
// ===================
const commands = [
    { name: 'claim', description: 'Claim the current ticket (Staff Only).' },
    { name: 'close', description: 'Close the current ticket (Staff Only).' },
    {
        name: 'ticket',
        description: 'Ticket management commands.',
        options: [
            { name: 'add', description: 'Add a user.', type: ApplicationCommandOptionType.Subcommand, options: [{ name: 'user', description: 'User.', type: ApplicationCommandOptionType.User, required: true }] },
            { name: 'remove', description: 'Remove a user.', type: ApplicationCommandOptionType.Subcommand, options: [{ name: 'user', description: 'User.', type: ApplicationCommandOptionType.User, required: true }] },
            { name: 'move', description: 'Move ticket.', type: ApplicationCommandOptionType.Subcommand, options: [{ name: 'category_key', description: 'Team.', type: ApplicationCommandOptionType.String, required: true, choices: Object.values(categories).map(c => ({ name: c.name, value: c.key })) }] },
            { name: 'rename', description: 'Rename ticket.', type: ApplicationCommandOptionType.Subcommand, options: [{ name: 'name', description: 'Name.', type: ApplicationCommandOptionType.String, required: true }] },
        ],
    },
];
...
client.once('ready', async () => {
    loadTickets(); 
    console.log(`🤖 ${client.user.tag} is online and watching!`);
